...
### Added
- **Pausable / resumable ingestion.** Pause a running ingest per source — or all at once ("Pause all", e.g. before bed or on heavy CPU) — from the Sources panel; **Resume** continues where it left off (incremental: already-finished files are hash-skipped). New `POST /api/ingest/pause` and `/api/ingest/resume`; the launcher ring shows amber while paused. Cooperative pause: stops at a file boundary and leaves any interrupted file `partial` so resume reprocesses it. Background watcher re-scans are pausable too.
