...
- **`/do dev clear`** *(web only)* — a one-line **clean slate** for demos: wipes every chunk, vector, source registration, and chat/audit history from the database, then resets the UI in place — no browser reload. It's two-step by design (you reply `--confirmed`) and **never deletes your source files on disk**, your API credentials, your PIN, or your Telegram pairing. Handy before handing the screen to someone new.
...
This workflow is **live in the web chat and on the private Telegram bot** — the same commands, and the same conversation: focus and the working-set are keyed by the session, so a thread started on your phone continues in the browser (a sidebar marks the one **📱 active on Telegram** and offers **"Continue on phone"** the other way). It runs identically on **macOS, Linux, and Windows** — the dispatcher, fuzzy search, PIN, focus, and OCR are cross-platform; only the verb *script* is OS-native (`#!/bin/sh` on macOS/Linux, PowerShell `.ps1` on Windows). Add your own verb against a documented contract + adversarial test protocol: **[`docs/agent/DO.md`](docs/agent/DO.md)**, **[`docs/agent/do-spec.md`](docs/agent/do-spec.md)**, runnable examples in [`examples/do/`](examples/do/).
